...
fn capture(word: &str, re_digits: Regex) -> String {
    ...
    let Some(match_begin) = re_numbers.captures(&result) else {panic!()};
    return match_begin[0].to_string();
}

fn main() {
    let contents = BufReader::new(File::open("./input").expect("Should have been able to read the file"));

    let mut sum = 0;
    for line in contents.lines() {
        for word in line.unwrap().split_whitespace() {
            let re_digits = Regex::new(r"(one|two|three|four|five|six|seven|eight|nine)").unwrap();
            let re_digits_rev = Regex::new(r"(eno|owt|eerht|ruof|evif|xis|neves|thgie|enin)").unwrap();
