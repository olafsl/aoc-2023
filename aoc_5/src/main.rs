...
fn string_to_ranges(string: &str) -> Vec<(isize, isize, isize)> {
    ...
    for transform in string.lines().skip(1) {
        let [destination, source, range] = transform
            .split_whitespace()
...
fn main() {
    ...
    let Ok(file) = filename_to_string("./src/input") else {
    ...
    let seeds = chunks[0]
        ...
        .map(|x| x.parse::<isize>().unwrap())
        .collect::<Vec<isize>>();
    let transforms = chunks[1..]
        .iter()
        .map(|x| string_to_ranges(x))
        .collect::<Vec<Vec<(isize, isize, isize)>>>();
    let mut lowest_loc = isize::MAX;

    let ranges = seeds.chunks(2).collect::<Vec<_>>();
    for range in ranges {
